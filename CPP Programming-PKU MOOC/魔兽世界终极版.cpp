...
如果武士在战斗中杀死敌人（不论是主动进攻杀死还是反击杀死），则其司令部会立即向其发送8个生命元作为奖励，使其生命值增加8。当然前提是司令部得有8个生命元。
...
拥有bomb的武士，在战斗开始前如果判断自己将被杀死（不论主动攻击敌人，或者被敌人主动攻击都可能导致自己被杀死，而且假设武士可以知道敌人的攻击力和生命值），那么就会使用bomb和敌人同归于尽。
武士不预测对方是否会使用bomb。武士使用bomb和敌人同归于尽的情况下，不算是一场战斗，双方都不能拿走城市的生命元，也不影响城市的旗帜。
...
lion 有“忠诚度”这个属性，其初始值等于它降生之后其司令部剩余生命元的数目。每经过一场未能杀死敌人的战斗，忠诚度就降低K。忠诚度降至0或0以下，则该lion逃离战场,永远消失。
但是已经到达敌人司令部的lion不会逃跑。Lion在己方司令部可能逃跑。lion 若是战死，则其战斗前的生命值就会转移到对手身上。所谓“战斗前”，就是每个小时的40分前的一瞬间。
...
在每个小时的第40分：在有两个武士的城市，会发生战斗。 如果敌人在5分钟前已经被飞来的arrow射死，那么仍然视为发生了一场战斗，而且存活者视为获得了战斗的胜利。
此情况下不会有“武士主动攻击”，“武士反击”，“武士战死”的事件发生，但战斗胜利后应该发生的事情都会发生。如Wolf一样能缴获武器，旗帜也可能更换，等等。在此情况下,Dragon同样会通过判断是否应该轮到自己主动攻击来决定是否欢呼。
...
public:
    ...
    virtual void attacked(unit & attacker) = 0;
    virtual void counter(unit & attacker) = 0;
    friend class headquarter;
    friend void event(int time);
    bool HPcheck(){
        if(hp > 0)
            return true;
        else
...
unit* redUnit[10000],*blueUnit[10000];
int redUnitCount=0,blueUnitCount=0;

class location{
protected:
    int HP = 0;
    int coordination;
    unit *slot1 = nullptr,*slot2 = nullptr;
public:
    friend void event(int time);
};

class headquarter:public location{
public:
    void sendReward(unit & target){
        if(HP >= 8){
            target.hp += 8;
            HP -= 8;
        }
    ...
    bool check(){
        if(slot1 != nullptr && slot2 != nullptr && slot1->faction == slot2->faction)
            return false;
        else
            return true;
    };
...
void event(int time){
    ...
    //狮子逃跑
    if(time == 5){
        for(int i = 0;i<redUnitCount;i++)
            if(redUnit[i]->type == "lion")
                redUnit[i]->hp = 0;
        for(int i = 0;i<blueUnitCount;i++)
            if(blueUnit[i]->type == "lion")
                blueUnit[i]->hp = 0;
    }
    //
}
